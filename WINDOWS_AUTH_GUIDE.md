...
```bash
$env:MSSQL_SERVER = "."
$env:MSSQL_DATABASE = "MyDatabase_LOCAL"
...
```bash
sqlcmd -S . -d MyDatabase_LOCAL -E -Q "SELECT @@VERSION"
```
...
```bash
$env:MSSQL_CONNECTION_STRING = "server=.;database=MyDatabase_LOCAL;integrated security=SSPI;encrypt=true;trustservercertificate=true"
```

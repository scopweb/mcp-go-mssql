...
func setupTestEnv() {
	...
	if os.Getenv("MSSQL_SERVER") == "" {
		os.Setenv("MSSQL_SERVER", "your-server.local")
	}
	if os.Getenv("MSSQL_DATABASE") == "" {
		os.Setenv("MSSQL_DATABASE", "MyDatabase")
	}
	if os.Getenv("MSSQL_USER") == "" {
		os.Setenv("MSSQL_USER", "myUser")
	}
	if os.Getenv("MSSQL_PASSWORD") == "" {
		os.Setenv("MSSQL_PASSWORD", "YourPassword")
	}
...
func TestBuildSecureConnectionString(t *testing.T) {
	...
	t.Run("Valid configuration", func(t *testing.T) {
		...
		if !strings.Contains(connStr, "server=your-server.local") {
			t.Errorf("Connection string should contain server")
		}
		if !strings.Contains(connStr, "database=MyDatabase") {

...
Use this as a starting point for a lightweight CI that validates the MCP server, runs security checks, and prevents leaking secrets.
...
jobs:
   build:
      ...
      steps:
         ...
         - name: Run gosec
            run: |
               sudo apt-get update && sudo apt-get install -y golang-gosec
               gosec ./...
         - name: Run govulncheck
            run: go install golang.org/x/vuln/cmd/govulncheck@latest && govulncheck ./...
         - name: Run security-check script (PowerShell on Windows)
            uses: mikeal/powershell@v2
            with:
               pwsh: true
               script: |
      ...
      steps:
         ...
         - name: Build (Windows)
            run: |
               ...
               go build -ldflags "-w -s -H windowsgui" -o mcp-go-mssql-windows-amd64.exe

```

---

## Minimal Tool Schema (JSON) - Generic resource operation

```json
{
   "name": "resource_op",
   "description": "Perform a safe operation on a configured resource",
   "inputSchema": {
      "type": "object",
      "properties": {
         "resource_id": { "type": "string", "description": "Target resource" },
         "operation": { "type": "string", "enum": ["query","summary","status"] },
         "params": { "type": "object" }
      },
...
Add a short example: a tool that only supports non-destructive operations by default and requires a `confirm=true` flag for destructive ops.

```json
{
   "name": "resource_modify",
   "description": "Modify resource only when confirm=true and whitelisted",
   "inputSchema": {
      "type": "object",
      "properties": {
         "resource_id": { "type": "string" },
         "operation": { "type": "string", "enum": ["modify","reset","clear"] },

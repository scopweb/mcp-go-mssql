...
Al intentar buscar referencias a un objeto SQL (`PedidoCamioCarregaCamioAdmin` con estados 26/27) en la base de datos `JJP_CRM`, Claude fallaba con el error:
...
`
```
En JJP_CRM con >200 tablas/vistas, el resultado JSON podía superar 50 KB.
...
Para encontrar referencias a `PedidoCamioCarregaCamioAdmin` con estados 26/27 en JJP_CRM:
